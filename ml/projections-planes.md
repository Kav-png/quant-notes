...
A **hyperplane** is a flat subspace of dimension \(n-1\) inside \(\mathbb{R}^n\).
It is defined by:
\[
...
| Value of \(d\) | Meaning |
|---|---|
| \(d > 0\) | \(\mathbf{x}\) is on the \(\theta\)-side of the plane |
| \(d < 0\) | \(\mathbf{x}\) is on the opposite side |
| \(d = 0\) | \(\mathbf{x}\) lies **on** the plane |
...
### Intuition
Any point \(\mathbf{x}\) off the plane can be split into:
\[
\mathbf{x} = \underbrace{\mathbf{x}_{\text{proj}}}_{\text{lies on plane}} + \underbrace{d \cdot \hat{\theta}}_{\text{perpendicular "overhang"}}
\]
To project, we **remove the overhang**:
\[
...
|---|---|---|
| Plane equation | \(\theta \cdot \mathbf{x} + \theta_0 = 0\) | Defines the hyperplane |
| Normal vector | \(\theta\) | Points perpendicularly away from plane |
| Signed distance | \((\theta \cdot \mathbf{x} + \theta_0) / \|\theta\|\) | How far & which side |
| Projection | \(\mathbf{x} - \frac{\theta \cdot \mathbf{x} + \theta_0}{\|\theta\|^2}\theta\) | Nearest point on plane |
