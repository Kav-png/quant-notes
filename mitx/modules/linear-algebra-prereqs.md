...
**Definition:** The determinant of a 2×2 matrix $A = \begin{pmatrix} a & b \\ c & d \end{pmatrix}$ is:

$$\det(A) = ad - bc$$

For a 3×3 matrix $A = \begin{pmatrix} a & b & c \\ d & e & f \\ g & h & i \end{pmatrix}$, expand along the first row:

$$\det(A) = a(ei - fh) - b(di - fg) + c(dh - eg)$$

**What it tells you:**

- $\det(A) \neq 0$ — the matrix is **invertible** (non-singular); it doesn't collapse space
- $\det(A) = 0$ — the matrix is **singular**; it squashes space into a lower dimension and has no inverse
- $|\det(A)|$ — the factor by which the matrix scales areas (2D) or volumes (3D)

**Intuition:** Think of a 2×2 matrix as defining a parallelogram from its two column vectors. The determinant is the **signed area** of that parallelogram. If the columns are parallel (linearly dependent), the area is zero — that's a singular matrix.

**Why it matters here:** Finding eigenvalues requires solving $\det(M - \gamma I) = 0$. We're asking: for which values of $\gamma$ does $(M - \gamma I)$ become singular? Those are exactly the eigenvalues.

**Example — HW2 matrix:**

$$\det\begin{pmatrix} a & b \\ c & d \end{pmatrix} = ad - bc$$

$$\det\begin{pmatrix} 0 & \lambda \\ \lambda & 0 \end{pmatrix} = (0)(0) - (\lambda)(\lambda) = -\lambda^2$$

And for the shifted matrix $(M - \gamma I)$:

$$\det\begin{pmatrix} -\gamma & \lambda \\ \lambda & -\gamma \end{pmatrix} = (-\gamma)(-\gamma) - (\lambda)(\lambda) = \gamma^2 - \lambda^2$$

Setting this to zero gives $\gamma = \pm\lambda$ — the two eigenvalues.
...
**Why diagonalization works — the derivation:**

Start from the definition of eigenvectors. For each eigenvector $v_i$ with eigenvalue $\gamma_i$:

$$Mv_i = \gamma_i v_i$$

We have two eigenvectors $v_1$ and $v_2$, so two equations:

$$Mv_1 = \gamma_1 v_1, \qquad Mv_2 = \gamma_2 v_2$$

**Step 1 — Stack the eigenvectors as columns of $V$:**

$$V = \begin{pmatrix} | & | \\ v_1 & v_2 \\ | & | \end{pmatrix}$$

**Step 2 — Write $MV$ and observe what happens column by column:**

$$MV = M\begin{pmatrix} | & | \\ v_1 & v_2 \\ | & | \end{pmatrix} = \begin{pmatrix} | & | \\ Mv_1 & Mv_2 \\ | & | \end{pmatrix} = \begin{pmatrix} | & | \\ \gamma_1 v_1 & \gamma_2 v_2 \\ | & | \end{pmatrix}$$

**Step 3 — Rewrite the right-hand side using $V$ and $\Gamma$:**

$$\begin{pmatrix} | & | \\ \gamma_1 v_1 & \gamma_2 v_2 \\ | & | \end{pmatrix} = \begin{pmatrix} | & | \\ v_1 & v_2 \\ | & | \end{pmatrix} \begin{pmatrix} \gamma_1 & 0 \\ 0 & \gamma_2 \end{pmatrix} = V\Gamma$$
...
This is the diagonalization. It works **if and only if** $V$ is invertible — which requires the eigenvectors to be linearly independent (not parallel). For symmetric matrices like our $M$, eigenvectors corresponding to distinct eigenvalues are always orthogonal and thus always independent, so diagonalization is always possible.

**Why the change of variables $C'_t = V^{-1}C_t$ decouples the system:**

Starting from $C_t = MC_{t-1} + \sigma\eta_t$, substitute $M = V\Gamma V^{-1}$:
...
Since $\Gamma$ is diagonal, each component of $C'_t$ evolves independently — a fully decoupled system of AR(1)s. Diagonalization is precisely the algebraic trick that makes this possible.
